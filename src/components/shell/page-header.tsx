  ...
  return (
    <header className={cn("flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between", className)}>
      <div className="min-w-0">
        {eyebrow ? (
          <p className="text-xs font-medium uppercase tracking-[0.14em] text-ink-3">{eyebrow}</p>
        ) : null}
        <h1 className="mt-1.5 font-serif text-[30px] leading-[1.15] tracking-[-0.01em] text-ink text-balance sm:text-[36px]">
          {title}
        </h1>
        {description ? <div className="mt-2 max-w-2xl text-[15px] text-pretty text-ink-2">{description}</div> : null}
      </div>
