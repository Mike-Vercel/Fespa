 ...
 *
 * Modifiche per FESPA: testi in italiano, `topOffset` per l'header fisso della home,
 * nessun menu a tendina su touch (si tocca direttamente la lettera), React Compiler disattivato
 * per questo file ("use no memo"): il componente lavora su DOM e ref dentro un solo effetto.
 */
...
export type GlyphPortalProps = {
  ...
  annotations?: boolean;
  enterLabel?: string;
  /** Altezza di un header fisso sopra il portale (px): la scena resta sotto l'header. */
  topOffset?: number;
  /** Coda dopo il contenuto, con il campo ancora fisso dietro (es. una sfumatura verso la sezione successiva). */
  outro?: ReactNode;
  className?: string;
...
/** Caratteri con la loro posizione (in unità UTF-16) nella parola. */
function charactersOf(text: string): Array<{ char: string; index: number }> {
...
export default function GlyphPortal({
  ...
  const offset = Number.isFinite(topOffset) ? Math.max(0, topOffset) : 0;
  ...
  useLayoutEffect(() => {
    ...
    let active = true;
    let ready = false;
    // Siamo dentro useLayoutEffect, non nel render: la regola di purezza qui è un falso positivo.
    ...
    const readInk = () => {
      ...
      for (const char of Array.from(text)) {
        ...
        letters.push({
          index: charOffset,
          x: advances[charOffset] - m.actualBoundingBoxLeft,
        ...
        if (found) candidates.push({ ...found, x: found.x + advances[charOffset], index: charOffset });
    ...
    const position = () => {
      const origin = (root ? root.getBoundingClientRect().top + root.clientTop : 0) + offset;
    ...
    const layout = () => {
      ...
      const smallViewport = section.querySelector<HTMLElement>("[data-gp-viewport]")!.offsetHeight - offset;
  ...
  }, [text, focusChar, interactive, fontFamily, weight, length, offset, clipId, hasFront]);
  ...
  return (
    <section
      ...
      className={className}
      aria-label={text}
      style={{ "--gp-length": length, "--gp-characters": Array.from(text).length, "--gp-offset": `${offset}px`, ...style } as CSSProperties}
    >
      <style>{`
        ...
        ${q} [data-gp-outro]{position:relative;pointer-events:none;}
      ...
      <div data-gp-pin>
          ...
          <defs>
            <clipPath id={clipId} clipPathUnits="userSpaceOnUse">
              <text
                ...
                x="0"
                y="0"
                style={{ fontFamily, fontWeight: weight, fontSize: 100, fontKerning: "none", fontVariantLigatures: "none", letterSpacing: 0 }}
              >
                {text}
              </text>
            </clipPath>
        ...
        <div data-gp-choices role="radiogroup" aria-label="Scegli la lettera da cui entrare" inert>
          {characters.map(({ char, index }, i) => (
            <button
              ...
              key={index}
              aria-label={`${char}, lettera ${i + 1} di ${characters.length}`}
            />
          ))}
        </div>
        <label data-gp-touch-picker>
          <span style={{ position: "absolute", width: 1, height: 1, overflow: "hidden", clipPath: "inset(50%)" }}>Lettera d&apos;ingresso</span>
        ...
        <div data-gp-caption>
          <span data-gp-hint aria-hidden="true">
            {interactive ? "Scorri per entrare." : annotations ? "Un passaggio attraverso le lettere" : ""}
          </span>
      ...
      <div data-gp-content id={`${uid}-content`} tabIndex={-1}>
        {children ?? (
          <div>
            <h2 data-gp-default-title>Una lettera diventa un luogo.</h2>
            <p data-gp-default-copy>La forma si apre su ciò che viene dopo.</p>
          </div>
        )}
      </div>
      {outro ? (
        <div data-gp-outro aria-hidden="true">
          {outro}
        </div>
      ) : null}
    </section>
