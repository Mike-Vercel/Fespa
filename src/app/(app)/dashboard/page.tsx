...
export default async function DashboardPage() {
  ...
  const now = new Date();
  const dashboard = await getDashboard(context, now);

  return (
    <div className="flex flex-col gap-10 animate-rise-in">
      <DashboardWelcome greeting={dashboard.greeting} name={dashboard.coachFirstName} />
      <PageHeader
        className="dashboard-reveal dashboard-reveal--header"
        eyebrow={formatLongDate(dashboard.today)}
